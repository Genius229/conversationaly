...
export function SettingsModals({
  ...
  return <>
    ...
    {modals.modelSettings && (
      <div className="fixed inset-0 bg-[oklch(var(--scrim)/0.55)] backdrop-blur-[2px] flex items-center justify-center z-modal p-4">
        <div className="bg-elevated rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
          {/* Header */}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex space-x-2">
                    <select
                      className="px-3 py-2 text-sm bg-elevated border border-line rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-ring focus:border-info/40"
                    ...
                    <select
                      className="flex-1 px-3 py-2 text-sm bg-elevated border border-line rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-ring focus:border-info/40"
                ...
                {modelConfig.provider === 'ollama' && (
                  <div>
                    ...
                    {error && (
                      <div className="bg-danger-soft border border-danger/40 text-danger-ink px-4 py-3 rounded mb-4">
                        {error}
                      </div>
                    )}
          ...
          <div className="border-t p-6 flex justify-end">
            <button
              onClick={() => onClose('modelSettings')}
              className="px-4 py-2 text-sm font-medium text-brand-ink bg-brand rounded-md hover:bg-brand-hover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-ring"
            >
              Done
            </button>
          </div>
    ...
    {modals.deviceSettings && (
      <div className="fixed inset-0 bg-[oklch(var(--scrim)/0.55)] backdrop-blur-[2px] flex items-center justify-center z-modal">
        <div className="bg-elevated rounded-lg p-6 max-w-md w-full mx-4 shadow-xl">
    ...
    {modals.modelSelector && (
      <div className="fixed inset-0 bg-[oklch(var(--scrim)/0.55)] backdrop-blur-[2px] flex items-center justify-center z-modal">
        <div className="bg-elevated rounded-lg max-w-4xl w-full mx-4 shadow-xl max-h-[90vh] flex flex-col">
          {/* Fixed Header */}
          ...
          <div className="p-6 pt-4 border-t border-line flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <label className="relative inline-flex items-center cursor-pointer">
                ...
                />
                <div className="w-11 h-6 bg-ink/10 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-ring rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-line after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-elevated after:border-line after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-brand"></div>
              </label>
              <div>
            ...
            <button
              onClick={() => onClose('modelSelector')}
              className="px-4 py-2 text-sm font-medium text-ink bg-sunken rounded-md hover:bg-ink/10 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-line"
            >
    ...
    {modals.errorAlert && (
      <div className="fixed inset-0 bg-[oklch(var(--scrim)/0.55)] backdrop-blur-[2px] flex items-center justify-center z-modal">
        <Alert className="max-w-md mx-4 border-danger/40 bg-elevated shadow-xl">
          <AlertTitle className="text-danger-ink">Recording Stopped</AlertTitle>
          <AlertDescription className="text-danger-ink">
            {messages.errorAlert}
            <button
              onClick={() => onClose('errorAlert')}
              className="ml-2 text-danger-ink hover:text-danger-ink underline"
            >
              Dismiss
            </button>
          </AlertDescription>
    ...
    {modals.chunkDropWarning && (
      <div className="fixed inset-0 bg-[oklch(var(--scrim)/0.55)] backdrop-blur-[2px] flex items-center justify-center z-modal">
        <Alert className="max-w-lg mx-4 border-warn/40 bg-elevated shadow-xl">
          <AlertTitle className="text-warn-ink">Transcription Performance Warning</AlertTitle>
          <AlertDescription className="text-warn-ink">
            {messages.chunkDropWarning}
            <button
              onClick={() => onClose('chunkDropWarning')}
              className="ml-2 text-warn-ink hover:text-warn-ink underline"
            >
              Dismiss
            </button>
          </AlertDescription>
